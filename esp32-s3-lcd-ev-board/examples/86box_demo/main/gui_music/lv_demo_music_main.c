...
lv_obj_t * _lv_demo_music_main_create(lv_obj_t * parent)
{
    ...
    lv_obj_t * btn_back = lv_imgbtn_create(cont);
    lv_imgbtn_set_src(btn_back, LV_IMGBTN_STATE_RELEASED, NULL, &img_return2, NULL);
    lv_imgbtn_set_src(btn_back, LV_IMGBTN_STATE_CHECKED_RELEASED, NULL, &img_return2, NULL);
    ...
    lv_obj_add_flag(btn_back, LV_OBJ_FLAG_CHECKABLE);

    lv_obj_add_event_cb(btn_back, play_event_return_cb, LV_EVENT_CLICKED, NULL);
    lv_obj_add_flag(btn_back, LV_OBJ_FLAG_CLICKABLE);
    //lv_obj_set_width(play_obj, img_return2.header.w);
    //lv_obj_align(play_obj, LV_ALIGN_TOP_LEFT, 30, 200);
    lv_obj_set_size(btn_back, img_return2.header.w, 80);
    ...
    lv_obj_set_grid_cell(btn_back, LV_GRID_ALIGN_START, 0, 1, LV_ALIGN_CENTER, 1, 1);
    ...
    start_anim = true;
    timer =  lv_timer_create(stop_start_anim, INTRO_TIME + 6000, NULL);
    ...
    lv_obj_fade_in(btn_back, 1000, INTRO_TIME + 1000); 
    ...
    lv_obj_t * logo = lv_img_create(show_music_layer.lv_obj_layer);
    lv_img_set_src(logo, &icon_player_yellow);
    lv_obj_move_foreground(logo);

    //lv_obj_t * title = lv_label_create(lv_scr_act());
    lv_obj_t * title = lv_label_create(show_music_layer.lv_obj_layer);
...
void _lv_demo_music_album_passive_next()
{
    uint32_t id = audio_get_index();
    LV_LOG_USER("passive next, %d", id);
...
void _lv_demo_music_play(uint32_t id)
{
    ...
    current = audio_get_index();
    if(current != id){
        audio_play_index(id);
        LV_LOG_USER("audio_play_index:%d", id);
    }
    track_load(id);
...
{
    //LV_LOG_USER("time:%d, %d,%d", ((spectrum_len - spectrum_i) * 1000) / 30, spectrum_len, spectrum_i_pause);
    playing = true;
    ...
    lv_anim_init(&a);
    lv_anim_set_values(&a, spectrum_i, track_len*1000);
    lv_anim_set_exec_cb(&a, spectrum_anim_cb);
    lv_anim_set_var(&a, spectrum_obj);
    lv_anim_set_time(&a, ((track_len*1000 - spectrum_i)));
    ...
    LV_LOG_USER("track_id, %d, track_length:[%d-%d], duration:%d ms", track_id, spectrum_i, track_len, (track_len*1000 - spectrum_i));
...
static lv_obj_t * create_cont(lv_obj_t * parent)
{
    ...
    LV_LOG_USER("next, %d:%d",LV_HOR_RES,  LV_VER_RES + LV_DEMO_MUSIC_HANDLE_SIZE * 2);
...
static void track_load(uint32_t id)
{
    ...
    LV_LOG_USER("track_id:%d, spectrum_i:%d", id, spectrum_i);
...
static void spectrum_draw_event_cb(lv_event_t * e)
{
    ...
    //LV_LOG_USER("spectrum_draw_event_cb");
...
static lv_obj_t * album_img_create(lv_obj_t * parent)
{
    ...
    LV_LOG_USER("track_id:%d, spectrum_len:%d", track_id, spectrum_len);
...
static void play_event_return_cb(lv_event_t * e)
{
    ...
    #if 0   
    static int enterPage = 0;
    ctrl_Layer.lv_obj_user = &enterPage;
    lv_func_goto_layer(&show_music_layer, &ctrl_Layer);
    #else
    main_Layer.lv_obj_user = &show_music_layer;
    lv_func_goto_layer(&show_music_layer, &main_Layer);
    #endif
}
