...
audio_list_info_t *audio_list_info = NULL;
...
static esp_err_t audio_file_scan(char *base_path)
{
    ...
    audio_list_info = (audio_list_info_t *) malloc(audio_count * sizeof(audio_list_info_t));
    ESP_RETURN_ON_FALSE(NULL != audio_list_info, ESP_ERR_NO_MEM,
    ...
    for (size_t i = 0; i < audio_count; i++) {
        p_dirent = readdir(p_dir_stream);
        if (NULL != p_dirent) {
            (audio_list_info + i)->mp3name = malloc(sizeof(p_dirent->d_name));
            
            memset(full_name, 0, sizeof(full_name));
            strcpy(full_name, base_path);
            strcat(full_name, "/");
            strcat(full_name, p_dirent->d_name);
            printf("\r\n");
            (audio_list_info + i)->mp3Duration = mp3_play_time_cal(full_name);
            ESP_LOGI(TAG, "File : %s, mp3Duration:%d sec", \
                    strcpy((audio_list_info + i)->mp3name, p_dirent->d_name), (audio_list_info + i)->mp3Duration);
        } else {
            ESP_LOGE(TAG, "The file system may be corrupted");
            closedir(p_dir_stream);
            for (int j = i - 1; j >= 0; j--) {
                free((audio_list_info + i)->mp3name);
            }
            free(audio_list_info);
...
char *audio_get_name_from_index(size_t index, char *base_path)
{
    ...
    ESP_RETURN_ON_FALSE(NULL != audio_list_info, NULL,
        TAG, "Audio file not found");

    ESP_RETURN_ON_FALSE(NULL != (audio_list_info + index)->mp3name, NULL,
        TAG, "Audio file not found");

    if (NULL != base_path) {
        strcat(strcat(base_path, "/"), (audio_list_info + index)->mp3name);
    }

    return (audio_list_info + index)->mp3name;
}

/* **************** AUDIO DECODE **************** */
static QueueHandle_t audio_event_queue = NULL;
QueueHandle_t audio_adv_event_queue = NULL;
...
static esp_err_t aplay_mp3(const char *path)
{
    ...
    ESP_LOGI(TAG, "%s stack use in %d", __FUNCTION__, uxTaskGetStackHighWaterMark(NULL));
    ...
    if(NULL == mp3_decoder){
        ...
        ESP_LOGI(TAG, "mp3_decoder null, MP3InitDecoder");
    }
    else{
        ESP_LOGI(TAG, "aplay_mp3 %s", path);
    }
    ...
    if(NULL == read_buf){
        ...
        //read_buf = heap_caps_malloc(MAINBUF_SIZE, MALLOC_CAP_DEFAULT | MALLOC_CAP_INTERNAL);
        ESP_GOTO_ON_FALSE(NULL != read_buf, ESP_ERR_NO_MEM, clean_up,
            TAG, "Failed allocate read buffer");

        ESP_LOGI(TAG, "malloc read_buf %d", MAINBUF_SIZE);
    }

    if(NULL == output){
        output = malloc(1152 * sizeof(int16_t) * 2 + 4);
        //output =  heap_caps_malloc(1152 * sizeof(int16_t) * 2, MALLOC_CAP_DEFAULT | MALLOC_CAP_INTERNAL);
        ESP_GOTO_ON_FALSE(NULL != output, ESP_ERR_NO_MEM, clean_up,
            TAG, "Failed allocate output buffer");

        ESP_LOGI(TAG, "malloc output %d", 1152 * sizeof(int16_t) * 2 + 4);
    }
    ...
    if (sizeof(mp3_id3_header_v2_t) == fread(&tag, 1, sizeof(mp3_id3_header_v2_t), fp)) {
        if (memcmp("ID3", (const void *) &tag, sizeof(tag.header)) == 0) {
            ...
            ESP_LOGI(TAG, "tag_len:%d", tag_len - sizeof(mp3_id3_header_v2_t));
        } else {
            /* Not ID3 header */
            fseek(fp, 0, SEEK_SET);
    ...
    do {
        ...
        if (pdPASS == xQueueReceive(audio_event_queue, &audio_event, 0)) {
            ...
            if (AUDIO_EVENT_CHANGE == audio_event) {
                i2s_zero_dma_buffer(I2S_NUM_0);
                ret = ESP_FAIL;
                ESP_LOGI(TAG, "AUDIO_EVENT_CHANGE, total_frame:%d ", total_frame);
                goto clean_up;
            }
        }
        ...
        if (bytes_left < MAINBUF_SIZE) {
            ...
            ESP_GOTO_ON_FALSE(bytes_read > 0, ESP_OK, clean_up,
                TAG, "No data read from strorage device:%d", total_frame);
        ...
        if (offset >= 0) {
            ...
            if(frame_info.nChans ==  1) {
                size_t new_output_sample_count = frame_info.outputSamps * 2;//len == 2304

                // convert from back to front to allow conversion in-place
                int16_t *out = (int16_t*)output + new_output_sample_count;//+2304*2 = 4608
                int16_t *in = (int16_t*)output + frame_info.outputSamps;//1152*2 = 2304
            ...
            if (sample_rate != frame_info.samprate ||
                ...
                //ESP_LOGI(TAG, "audio info: sr=%d, bit=%d, ch=%d, outputSamps:%d", sample_rate, bits_cfg, nChans, frame_info.outputSamps);
...
clean_up:
    ESP_LOGI(TAG, "#### MP3 clean_up start ####");
    ESP_LOGI(TAG, "%s stack use out %d", __FUNCTION__, uxTaskGetStackHighWaterMark(NULL));
    ...
    //ESP_LOGI(TAG, "MP3FreeDecoder(mp3_decoder)");

    if (NULL != read_buf){}       //free(read_buf);
    //ESP_LOGI(TAG, "free(read_buf)");

    if (NULL != output){}         //free(output);
    //ESP_LOGI(TAG, "free(output)");

    if (NULL != fp){}             fclose(fp);
    //ESP_LOGI(TAG, "fclose(fp)");
    return ret;
}
...
{
    /* Scan audio file */
    adv_event_t event = ADV_EVENT_NONE;

    char *base_path = (char *) pvParam;
    ESP_LOGI(TAG, "base_path:%s", base_path);

    audio_adv_event_queue = xQueueCreate(4, sizeof(adv_event_t));
    if (NULL == audio_adv_event_queue) {
        ESP_LOGI(TAG, "audio_adv_event_queue failed");
        vTaskDelete(NULL);
    }
    ...
    if (NULL != base_path) {
        ...
        #else
        event = ADV_EVENT_FILE_SCAN_DONE;
        BaseType_t ret_val = xQueueSend(audio_adv_event_queue, &event, 0);
    ...
    while (vTaskDelay(1), NULL != file_name) {
        ...
        ESP_LOGI(TAG, "ret_valL%d, audio_index:%d, audio_count:%d", ret_val, audio_index, audio_count);
        ...
        ESP_LOGI(TAG, "prepare file_name:%s", file_name);
    }

    ESP_LOGI(TAG, "break, file_name:%s", file_name);
