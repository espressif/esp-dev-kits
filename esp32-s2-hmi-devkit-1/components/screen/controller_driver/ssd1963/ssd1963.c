...
static void lcd_ssd1963_init_reg(void)
{
	...
	LCD_WRITE_DATA(0x1D);		//参数1 
	LCD_WRITE_DATA(0x02);		//参数2 Divider M = 2, PLL = 300/(M+1) = 100MHz
	...
	LCD_WRITE_CMD(0xE6);		//设置像素频率,33Mhz
	LCD_WRITE_DATA(0x2F);
	LCD_WRITE_DATA(0xFF);
	LCD_WRITE_DATA(0xFF);
	...
	LCD_WRITE_DATA((SSD_HOR_RESOLUTION-1)>>8);//设置LCD水平像素
	LCD_WRITE_DATA(SSD_HOR_RESOLUTION-1);		 
	LCD_WRITE_DATA((SSD_VER_RESOLUTION-1)>>8);//设置LCD垂直像素
	LCD_WRITE_DATA(SSD_VER_RESOLUTION-1);		 
