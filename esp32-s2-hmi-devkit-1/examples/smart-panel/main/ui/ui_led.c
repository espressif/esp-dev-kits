...
static void led_color_update(void)
{
    ...
    if (led_on) {
        color.full = lv_color_hsv_to_rgb(lv_cpicker_get_hue(_color_sel), lv_slider_get_value(_slider_saturation), lv_slider_get_value(_slider_brightness)).full;

        ws2812_set_rgb(0, color.ch.red << 3, color.ch.green << 3, color.ch.blue << 3);
        ws2812_refresh();
    } else {
