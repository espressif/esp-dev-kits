...
static const char *TAG = "ui_thermostat";
...
static bool thermostat_layer_enter_cb(struct lv_layer_t * layer);
static bool thermostat_layer_exit_cb(struct lv_layer_t * layer);
...
static void thermostat_event_cb(lv_event_t *e)
{
    ...
    if (LV_EVENT_FOCUSED == code) {
        ...
        if(is_time_out(&time_500ms)){
            ...
            printf("evt=%s, temp_arc set value:[%d]\n", (LV_KEY_RIGHT == key)? "LV_KEY_RIGHT":"LV_KEY_LEFT", current);
        }
        else{
...
void lv_create_obj_roller(lv_obj_t * parent)
{
    ...
    lv_obj_set_style_text_line_space(temp_wheel, 50, LV_PART_MAIN);
    ...
    lv_roller_set_visible_row_count(temp_wheel, 1);
...
static bool thermostat_layer_enter_cb(struct lv_layer_t * layer)
{
	...
	if(NULL == layer->lv_obj_layer){
        ...
        ui_thermostat_init(layer->lv_obj_layer);
