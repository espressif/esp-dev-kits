...
void ui_light_2color_init(lv_obj_t * parent)
{
    ...
    img_pwm_set = lv_img_create(page);
    if(light_set_conf.light_pwm/25 < sizeof(light_pwm_image)/sizeof(uint8_t *)){
        lv_img_set_src(img_pwm_set, light_pwm_image[light_set_conf.light_pwm/25]);
    }
    lv_obj_align(img_pwm_set, LV_ALIGN_CENTER, 0, 65);
...
static void light_2color_layer_timer_cb(lv_timer_t * tmr)
{
    if(is_time_out(&time_20ms)){
        ...
        if((light_set_conf.light_pwm^light_xor.light_pwm) || (light_set_conf.light_cck^light_xor.light_cck)){
            ...
            printf("light_xor.light_pwm:%d, %d\r\n", light_xor.light_cck, light_xor.light_pwm);
            ...
            lv_obj_add_flag(img_light_pwm_0, LV_OBJ_FLAG_HIDDEN);
            if((light_xor.light_pwm/25) < sizeof(light_pwm_image)/sizeof(uint8_t *)){
                lv_img_set_src(img_pwm_set, light_pwm_image[light_xor.light_pwm/25]);
            }
