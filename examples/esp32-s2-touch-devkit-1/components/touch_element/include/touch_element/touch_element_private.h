...
/* -------------------------------------------- Board basic type --------------------------------------------- */
typedef te_state_t te_board_state_t;

typedef struct {
    touch_elem_dispatch_t dispatch_method;      //Slider dispatch method
    touch_board_callback_t callback;           //Slider callback routine
    uint32_t event_mask;                        //Slider subscribed event mask
    void *arg;                                  //User input argument
} te_board_handle_config_t;

struct te_board_s {
    te_board_handle_config_t *config;
    struct te_slider_s horizontal_slider;
    struct te_slider_s vertical_slider;
    te_board_state_t current_state;
    te_board_state_t last_state;
    touch_board_event_t event;
    touch_board_position_t position;
};

typedef struct te_board_s* te_board_handle_t;
/* ------------------------------------------------------------------------------------------------------------------ */
