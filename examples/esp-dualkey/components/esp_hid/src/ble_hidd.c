...
#define CCC_NAMESPACE "ccc_store"

static esp_err_t save_ccc_to_nvs(const uint8_t *bda, uint8_t ccc_value)
{
    nvs_handle_t handle;
    esp_err_t err = nvs_open(CCC_NAMESPACE, NVS_READWRITE, &handle);
    if (err != ESP_OK) {
        return err;
    }
    char key[20];
    ...
    err = nvs_set_u8(handle, key, ccc_value);
    ESP_LOGD(TAG, "Saving ccc: %s = 0x%02X (%s)", key, ccc_value, esp_err_to_name(err));
    if (err == ESP_OK) {
...
static bool load_ccc_from_nvs(const uint8_t *bda, uint8_t *ccc_val_out)
{
    nvs_handle_t handle;
    char key[32];
    snprintf(key, sizeof(key), "%02X%02X%02X%02X%02X%02X",
             bda[0], bda[1], bda[2], bda[3], bda[4], bda[5]);
    esp_err_t err = nvs_open(CCC_NAMESPACE, NVS_READONLY, &handle);
    if (err == ESP_OK) {
        uint8_t val = 0;
        if (nvs_get_u8(handle, key, &val) == ESP_OK) {
            ESP_LOGD(TAG, "Restored CCC %s = 0x%02X", key, val);
            *ccc_val_out = val;
            nvs_close(handle);
            return true;
        }
...
static void hid_event_handler(esp_ble_hidd_dev_t *dev, int device_index, esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_CONNECT_EVT: {
        ...
        if (!dev->connected && device_index == (dev->devices_len - 1)) {
            ...
            // from NVS restore CCC value
            for (int i = 0; i < dev->devices[device_index].reports_len; i++) {
                hidd_le_report_item_t *map = &dev->devices[device_index].reports[i];

                if (map->ccc_handle != 0) {
                    uint8_t ccc_val = 0;
                    if (load_ccc_from_nvs(param->connect.remote_bda, &ccc_val)) {
                        map->ccc.value = ccc_val;
                        map->ccc.notify_enable = ccc_val & 0x01;
                        map->ccc.indicate_enable = ccc_val & 0x02;

                        ESP_LOGD(TAG, "Restored CCC for map[%d]: 0x%02X (notify=%d, indicate=%d)",
                                 i, ccc_val, map->ccc.notify_enable, map->ccc.indicate_enable);
                    } else {
                        ESP_LOGD(TAG, "No CCC found in NVS for map[%d]", i);
                    }
                }
            }
        ...
        if (param->write.handle == dev->devices[device_index].hid_control_handle) {
            ...
            if (map) {
                if (param->write.handle == map->ccc_handle) {
                    ...
                    save_ccc_to_nvs(dev->remote_bda, param->write.value[0]);
