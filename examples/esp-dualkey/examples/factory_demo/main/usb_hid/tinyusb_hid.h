 ...
 *
 * @param report HID report structure containing keyboard data
 */
void tinyusb_hid_keyboard_report(hid_report_t report);

/**
 * @brief Report mouse movement and scroll wheel actions.
 *
 * @param report HID report structure containing mouse data
 */
void tinyusb_hid_mouse_report(hid_report_t report);

/**
 * @brief Send VIA Raw HID response data to host.
 *
 * @param data Pointer to response data buffer
 * @param len Length of data (max 32 bytes)
 */
void tinyusb_hid_send_via_response(uint8_t *data, uint16_t len);

/**
 * @brief Get keycode from VIA keymap.
 *
 * @param layer Layer index (0-3)
 * @param row Row index (0 for ESP-dualkey)
 * @param col Column index (0-1 for ESP-dualkey)
 * @return uint16_t Keycode, or 0 if invalid position
 */
