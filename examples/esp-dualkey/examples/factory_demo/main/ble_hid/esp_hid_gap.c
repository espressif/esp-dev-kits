...
static void ble_gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_AUTH_CMPL_EVT:
        if (!param->ble_security.auth_cmpl.success) {
            ...
            esp_ble_conn_update_params_t conn_params = {
                .min_int = 0x20, //interval_min unit:1.25ms Range: 0x06 to 0x0C80
                .max_int = 0x20, //interval_max unit:1.25ms Range: 0x06 to 0x0C80
                .latency = 0x5, //latency Slave latency for the connection in number of connection events. Range: 0x0000 to 0x01F3
                .timeout = 0x100, //Supervision timeout for the LE Link. Range: 0x000A to 0x0C80. 0x000A to 0x0C80 Time = N * 10 msec
            };
            memcpy(conn_params.bda, param->ble_security.ble_req.bd_addr, ESP_BD_ADDR_LEN);
            esp_ble_gap_update_conn_params(&conn_params);
            ESP_LOGI(TAG, "esp_ble_gap_update_conn_params int_min:%d int_max:%d latency:%d timeout:%d", conn_params.min_int, conn_params.max_int, conn_params.latency, conn_params.timeout);
        }
        break;
    ...
    case ESP_GAP_BLE_UPDATE_CONN_PARAMS_EVT:
        ...
        bsp_get_battery_state(&is_charging, &battery_voltage);
        ble_hid_battery_update(BATTERY_VOLTAGE_TO_PERCENTAGE(battery_voltage));
        ESP_LOGI(TAG, "CONNECT, battery_voltage: %d", BATTERY_VOLTAGE_TO_PERCENTAGE(battery_voltage));
        ESP_LOGI(TAG, "BLE GAP UPDATE_CONN_PARAMS status:%d min:%d max: %d", param->update_conn_params.status, param->update_conn_params.min_int, param->update_conn_params.max_int);
        break;
    case ESP_GAP_BLE_SET_PKT_LENGTH_COMPLETE_EVT:
        ESP_LOGI(TAG, "BLE GAP SET_PKT_LENGTH_COMPLETE status:%d rx_len:%d tx_len:%d", param->pkt_data_length_cmpl.status, param->pkt_data_length_cmpl.params.rx_len, param->pkt_data_length_cmpl.params.tx_len);
        break;
    default:
...
esp_err_t esp_hid_ble_gap_adv_start(void)
{
    ...
    // Get local BLE MAC address and print it
    uint8_t mac[6] = {0};
    ...
    if (err == ESP_OK) {
        ...
        ESP_LOGE(TAG, "Failed to get BLE MAC address, err=0x%x", err);
    }
    // Make the BLE MAC address conform to the BLE random address specification
    // Set the two most significant bits of the first byte to '1' and '0' respectively (static random address: 0b11xxxxxx)
    mac[0] |= 0xC0; // Set bits 7 and 6 to 1 (0xC0 = 11000000b)
    mac[5] = mac[5] + 1;  // Add 1 to the least significant byte to make it different from the public address
    esp_ble_gap_set_rand_addr(mac);
    hidd_adv_params.own_addr_type = BLE_ADDR_TYPE_RANDOM;
