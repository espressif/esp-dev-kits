...
static esp_lv_adapter_tear_avoid_mode_t get_tear_avoid_mode(void)
{
...
#elif CONFIG_BSP_DISPLAY_LVGL_FULL_REFRESH
#if CONFIG_BSP_LCD_RGB_BUFFER_NUMS >= 3
    return ESP_LV_ADAPTER_TEAR_AVOID_MODE_TRIPLE_FULL;
#else
...
lv_display_t *lvgl_adapter_init(const bsp_display_config_t *cfg)
{
    ...
    const esp_lv_adapter_tear_avoid_mode_t tear_avoid_mode = get_tear_avoid_mode();
    ...
    disp_cfg.profile.buffer_height = CONFIG_BSP_DISPLAY_LVGL_BUF_HEIGHT;
    ...
    ESP_LOGI(TAG, "LVGL adapter started: %ux%u, tear_mode=%d, buffer_height=%u, use_psram=%d",
             BSP_LCD_H_RES, BSP_LCD_V_RES, tear_avoid_mode, disp_cfg.profile.buffer_height, disp_cfg.profile.use_psram);
    return disp;
}
