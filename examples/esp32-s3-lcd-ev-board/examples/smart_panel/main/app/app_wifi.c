...
static void wifi_scan(void)
{
    ...
    if (ap_count && (ESP_OK == ret)) {
        scan_result.ap_count = (ap_count < DEFAULT_SCAN_LIST_SIZE) ? ap_count : DEFAULT_SCAN_LIST_SIZE;
        memcpy(&scan_result.ap_info[0], &ap_info[0], sizeof(wifi_ap_record_t)*scan_result.ap_count);
    } else {
...
static void network_task(void *args)
{
    ...
    while (1) {
        ...
        if ((xTaskGetTickCount() - tick) > (30 * 60 * 1000)) {
            tick = xTaskGetTickCount();
            if (wifi_connected) {
...
{
    assert(scan_result.wifi_mux && "bsp_display_start must be called first");

    const TickType_t timeout_ticks = (timeout_ms == 0) ? portMAX_DELAY : pdMS_TO_TICKS(timeout_ms);
    return xSemaphoreTakeRecursive(scan_result.wifi_mux, timeout_ticks) == pdTRUE;
}

void app_wifi_unlock(void)
{
    assert(scan_result.wifi_mux && "bsp_display_start must be called first");
    xSemaphoreGiveRecursive(scan_result.wifi_mux);
}

void app_wifi_state_set(wifi_scan_status_t status)
{
    app_wifi_lock(0);
    scan_result.scan_done = status;
    app_wifi_unlock();
}
...
void app_network_start(void)
{
    ...
    scan_result.wifi_mux = xSemaphoreCreateRecursiveMutex();
    ESP_ERROR_CHECK_WITHOUT_ABORT((scan_result.wifi_mux) ? ESP_OK : ESP_FAIL);
