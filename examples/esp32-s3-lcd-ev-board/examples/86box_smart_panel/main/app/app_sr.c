...
{
    if (pdTRUE != xQueueSend(g_sr_data->result_que, &result, 0)) {
        ESP_LOGE(TAG, "handle info failed");
        return ESP_OK;
    } else {
        return ESP_FAIL;
    }
...
{
    int size = 4 * 2 * 32 * 16;   // It's 32ms for 4 channels, 4k bytes //AFE_CONFIG_DEFAULT
...
esp_err_t app_sr_start(bool record_en)
{
    ...
    g_sr_data->result_que = xQueueCreate(1, sizeof(sr_result_t));
    ...
    if (file_save[0] == NULL) {
        ESP_LOGI(TAG, "can not open file\n");
    }
    ...
    FILE *file_test = fopen("/sdcard/fopen.txt", "w");
    if (file_test == NULL) {
        ESP_LOGI(TAG, "can not open file\n");
    } else {
        char *bufwrite = "sd card test";
        //size_t fwrite(constvoid*buffer,size_tsize,size_tcount,FILE*file);
        int ret = fwrite(bufwrite, 1, strlen(bufwrite), file_test);
        ESP_LOGI(TAG, "fwrite:%d, len:%d\n", ret, strlen(bufwrite));
        fclose(file_test);
    }
...
esp_err_t app_sr_modify_cmd(uint32_t id, const sr_cmd_t *cmd)
{
    ...
    SLIST_FOREACH(it, &g_sr_data->cmd_list, next) {
        if (it->id == id) {
            ESP_LOGI(TAG, "modify cmd [%"PRIu32"] from %s to %s", id, it->str, cmd->str);
...
esp_err_t app_sr_remove_cmd(uint32_t id)
{
    ...
    SLIST_FOREACH(it, &g_sr_data->cmd_list, next) {
        if (it->id == id) {
            ESP_LOGI(TAG, "remove cmd id [%"PRIu32"]", it->id);
